...
export default function Login() {
  const { loginWithGoogle, loginWithEmail, requestPasswordReset } = useAuth();
  ...
  const onReset = async () => {
    ...
    try {
      ...
      alert("If an account exists for this email, we've sent a reset link.");
    } catch (err) {
    ...
    <>
      ...
      <div className="wv-auth-shell">
        <div className="wv-card">
          <h1 className="wv-title">Log in to WellVantage</h1>

          <form onSubmit={onEmailLogin}>
            <label className="wv-label" htmlFor="email">Username or Email</label>
            <input
              id="email"
              className="wv-input"
              type="email"
          ...
          <div className="wv-muted">
            <button className="wv-linkbtn" onClick={onReset}>Forgot password?</button>
          </div>

          <div className="wv-bottom">
            Don’t have a WellVantage account?{" "}
            <Link to="/signup" className="wv-link">Sign Up</Link>
          </div>
        </div>
      </div>
    </>
